/* Fetched from github.com/RezwanArefin01/CodeTemplate */
...
void buildY(int ndx, int lx, int rx, int ndy, int ly, int ry) {
	if(ly == ry) {
		...
		else tree[ndx][ndy] = tree[ndx*2][ndy] + tree[ndx*2+1][ndy]; // merge
		return;
	} int mid = ly + ry >> 1;
	buildY(ndx, lx, rx, ndy*2, ly, mid);
	buildY(ndx, lx, rx, ndy*2+1, mid+1, ry);
	tree[ndx][ndy] = tree[ndx][ndy*2] + tree[ndx][ndy*2+1]; // merge
}
void buildX(int ndx, int lx, int rx) {
	...
	} buildY(ndx, lx, rx, 1, 0, m-1); // edit indexing
}
...
void updateX(int ndx, int lx, int rx, int x, int y, int val) {
	...
	} updateY(ndx, lx, rx, 1, 0, m-1, y,val); // edit indexing
}
...
int queryY(int ndx, int ndy, int ly, int ry, int y1, int y2) {
	...
	return queryY(ndx, ndy*2, ly, mid, y1, y2) + 
		   queryY(ndx, ndy*2+1, mid+1, ry, y1, y2); // merge
}
int queryX(int ndx, int lx, int rx, int x1, int y1, int x2, int y2) {
	if(rx < x1 || lx > x2) return 0;
	if(x1 <= lx && rx <= x2) {
		return queryY(ndx, 1, 0, m-1, y1, y2); // edit indexing
	} int mid = lx + rx >> 1;
	return queryX(ndx*2, lx, mid, x1,y1,x2,y2) +
		   queryX(ndx*2+1, mid+1, rx, x1,y1,x2,y2); // merge
}
