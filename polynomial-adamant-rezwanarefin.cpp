...
// source: https://github.com/RezwanArefin01/CodeTemplate/blob/master/Fast%20Fourier%20Transform.cpp
...
namespace algebra {
  ...
  namespace fft {
    ...
    void mul(vector<T> &a, const vector<T> &b) {
      int n = a.size();
      int m = b.size();
      if(!n or !m) return void(a.clear());
      if(m == 1) {for(T& x : a) {x *= b[0];}; return;}
      if(n == 1) {
        T scaler = a[0]; a = b;
        for(T& x : a) x *= b[0];
        return;
  ...
  struct poly {
    ...
    poly pow(size_t k, size_t n) { // calculate p^k(n) mod x^n, practically useless though
      poly b = *this, ret({1});
      while(k) {
        if(k & 1) ret = (ret * b).mod_xk(n);
        b = (b * b).mod_xk(n), k >>= 1;
      } return ret;
      /*if(is_zero()) {
        return *this;
